...
AgentHub 旨在打造一个统一的桌面应用，让开发者能够在一个界面中连接任意环境、启动任意 AI 终端代理，实现"一个入口，管理所有 AI 编程助手"。
...
AgentHub 是一款面向开发者的跨平台桌面应用，定位为 **AI 终端代理的统一管理中心**。它不是一个新的 AI Agent，而是所有现有 CLI Agent 的"超级终端"——提供统一的环境连接、Agent 启动、会话管理和配置管理能力。
...
除内置预设外，用户可以注册自定义 Agent。只需提供命令名、参数、环境变量即可。这使得 AgentHub 能够支持未来出现的任何新的 CLI Agent，无需等待应用更新。

---

## 8. 前端界面设计

本节以 `agreedysquid-ui-v5.jsx` 中的交互原型为准，统一说明 AgentHub 的界面信息架构与关键交互。

### 8.1 整体布局（桌面端）

```
┌──────────────────────────────────────────────────────────────────────────┐
│  🦑 agreedysquid / AgentHub                                  ⚙ ─ □ ✕     │  ← Titlebar
├───────────────┬───────────────────────────────────────────────────────────┤
...
| 潜在竞品 | 差异化策略 |
|----------|-----------|
| 各 Agent 厂商自建 GUI | AgentHub 的价值在于"统一"——不绑定任何单一厂商 |
| VS Code 终端插件 | AgentHub 更轻量、专注，不依赖 IDE；支持独立窗口和多服务器管理 |
| Warp / iTerm2 等终端 | 这些是通用终端，AgentHub 专注 AI Agent 场景，提供 Agent 感知的增强功能 |
